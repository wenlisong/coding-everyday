...
class MagicDictionary {
    ...
    public List<String> generalizedString(String word) {
    ...
    public void buildDict(String[] dict) {
        for (String word : dict) {
            this.words.add(word);
            for (String generalWord : generalizedString(word)) {
                this.counts.put(generalWord, this.counts.getOrDefault(generalWord, 0) + 1);
            }
        }
    }
    ...
    public boolean search(String word) {
        for (String generalWord : generalizedString(word)) {
            int count = this.counts.getOrDefault(generalWord, 0);
            if (count > 1 || (count == 1 && !this.words.contains(word))) {
                return true;
            }
        }
        return false;
